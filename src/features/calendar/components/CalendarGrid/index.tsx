...
export default function CalendarGrid({ year, month, todos, selectedDay, onSelectDay }: CalendarGridProps) {
  ...
  return (
    <div className="w-full overflow-hidden last:rounded-b-4xl">
      {/* 요일 헤더 */}
      <div className="border-border-secondary dark:border-[#4F4F4F] grid grid-cols-7 border-t border-b">
        {DAYS.map((d) => (
          <div
            key={d}
            className="border-border-secondary dark:border-[#4F4F4F] border-r py-2 text-center text-xs font-medium text-gray-500 dark:text-[#BABABA] last:border-r-0"
          >
            {d}
          </div>
        ))}
